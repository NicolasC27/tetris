...
** Last update Sun Mar 20 20:37:24 2016 romain samuel
*/
...
int		game(t_tetris *s)
{
  ...
  if (create_compartments(s) == -1)
    return (-1);
  s->scene->start_level = s->scene->level;
  s->game.current.coord = NULL;
  s->game.current.simple_coord = NULL;
  game_loop(s);
  free_matrix(s);
  return (0);
}
